...
from .attn_rsn_pl import (gen_mask, MultiheadAttention)
...
#%%
class AttnRsnClassic(Classifier):
    def __init__(self, param_model, random_state=0, pos_weight=None):
        # param_model.dim_in=1
        # param_model.dim_out=1
        super(AttnRsnClassic, self).__init__(param_model, random_state, pos_weight)
...
class AttnResNet(nn.Module):
    def __init__(self, block, num_blocks, input_channel, output_size, stride, first_ksize, 
                       kernel_size, base_filters, groups=1, is_se=False,
                       n_mix_block=None, alpha=None, num_heads=8, emb_dim=320):
        super(AttnResNet, self).__init__()

        self.is_se = is_se
        self.n_leads = input_channel
        ...
        self.layer2 = self._make_layer(block, base_filters*2, num_blocks[1], kernel_size=kernel_size, stride=stride, is_se=self.is_se, groups=groups)
        self.layer3 = self._make_layer(block, base_filters*4, num_blocks[2], kernel_size=kernel_size, stride=stride, is_se=self.is_se, groups=groups)
        self.layer4 = self._make_layer(block, base_filters*8, num_blocks[3], kernel_size=kernel_size, stride=stride, is_se=self.is_se, groups=groups)
        ...
        out_channels = base_filters*8*block.expansion
        self.multihead_attn = MultiheadAttention(out_channels//self.n_leads, emb_dim, num_heads)

        # Classifier
        # self.main_clf = nn.Linear(base_filters*8*block.expansion, output_size)
        self.main_clf = nn.Linear(emb_dim//output_size, 1)
    ...
    def forward(self, x):
        ...
        out = out.squeeze(-1)
        out = out.view(out.shape[0], self.n_leads, -1) # (n_batch, n_leads, lead_channels)
        # print('Resnet output', out.shape)

        # # Multihead Attention
        mask = gen_mask(valid_leads, self.num_heads).to(out.device)
        attn_out = self.multihead_attn(out, mask=mask) #  (n_batch, emb_dim)
        # print('Attention output', attn_out.shape)
        out = attn_out.view(attn_out.shape[0], self.output_size, -1) #  (n_batch, output_size, emb_dim//output_size)
        # print('Attention output after view', out.shape)
        out = self.main_clf(out).squeeze(-1)
        # print('Clf output', out.shape)
        return out
        
def ResNet18(input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se, groups,num_heads, emb_dim):
    return AttnResNet(BasicBlock, [2,2,2,2], input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se=is_se, groups=groups,num_heads=num_heads, emb_dim=emb_dim)
    
def ResNet34(input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se, groups,num_heads, emb_dim):
    return AttnResNet(BasicBlock, [3,4,6,3], input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se=is_se, groups=groups,num_heads=num_heads, emb_dim=emb_dim)

def ResNet50(input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se, groups,num_heads, emb_dim):
    return AttnResNet(Bottleneck, [2,2,2,2], input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se=is_se, groups=groups,num_heads=num_heads, emb_dim=emb_dim)

def ResNet101(input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se, groups,num_heads, emb_dim):
    return AttnResNet(Bottleneck, [3,4,23,3], input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se=is_se, groups=groups,num_heads=num_heads, emb_dim=emb_dim)

def ResNet152(input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se, groups,num_heads, emb_dim):
    return AttnResNet(Bottleneck, [3,8,36,3], input_channel, output_size, stride, first_ksize, kernel_size, base_filters, is_se=is_se, groups=groups,num_heads=num_heads, emb_dim=emb_dim)
